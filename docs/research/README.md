...
| [Steamforged store split](2026-08-12-steamforged-warmachine-store-split.md) | `mfr-steamforged` fell 600 → 248 observations and failed `minCount`. **Not a scraper break and not a catalogue cut:** Steamforged moved Warmachine, P3 Paints and Iron Kingdoms to a second store it owns, **warmachine.gg** (613 products). All **352** missing products matched there by sku, 350 by identical handle — despite 404ing on steamforged.com with **zero** re-tags. Added `mfr-warmachine`; lowered the steamforged floor to 210 for genuine scope loss; corrected the taxonomy's exclusion of P3/Iron Kingdoms (Steamforged-owned brands, not licensed IP). |
| [Arcturus 2025 rewrite audit](2026-08-14-arcturus-2025-rewrite-audit.md) | Upstream's 2025-03-27 rewrite touched **every** brand file, and PR #132 had already traced 37 phantom P3 rows to it. **It did not do the same anywhere else:** its 1,531 added names are better evidenced than the rows they joined — Army Painter's additions carry a barcode **92%** of the time against **51%** for the pre-existing baseline. Only 23 added rows lack all three of barcode/code/harvest, and 12 of those are in a brand with 0% baseline coverage. What it did leave is **four Army Painter paints published twice**, once under a misspelling the harvest's name-match then missed by one letter (`Boney`/`Bony Spikes`). Two findings left open: Scale75's Artist Range carries a *pre-existing* misspelling cluster (`Yellow Ocre`, `Crimsom`, `Prusian Blue`), and Kimera's 12 signature-set rows are unverified in both directions. |
