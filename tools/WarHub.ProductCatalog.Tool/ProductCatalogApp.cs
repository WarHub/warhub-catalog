...
namespace WarHub.ProductCatalog.Tool;

/// <summary>
/// The product catalog tool's CLI entrypoint, extracted from <c>Program.cs</c> so it can be
/// invoked in-process from tests (see CliEndToEndTests) as well as from the real CLI.
/// </summary>
internal static class ProductCatalogApp
{
    ...
    /// </summary>
    /// <remarks>This mirrors the logic exercised directly by
    /// WarHub.ProductCatalog.Tool.Tests.Integration.ManufacturerCompleteScrapedTotalTests, which
    /// asserts the resulting dictionary and the guard decision built from it.</remarks>
    ...
    private static async Task<IReadOnlyList<RawProduct>> FetchGamesWorkshopProducts(
    ...
    private static async Task<IReadOnlyList<RawProduct>> FetchParaBellumProducts(
    ...
    /// </summary>
    private static async Task<(Dictionary<string, (string? Ean, string Source)> EanLookup, HashSet<string> CatalogSkus)> LoadExistingCatalogDataAsync(
    ...
    /// </summary>
    private static async Task<IReadOnlyList<Product>> LoadExistingFactionProductsAsync(string factionPath, CancellationToken ct)
    {
    ...
    /// </summary>
    private static IReadOnlyList<Product> ApplyShopifyEans(
    ...
    private static async Task<IReadOnlyList<RawProduct>> FetchFromMultiStoreCache(
    ...
    private static async Task<IReadOnlyList<RawProduct>> FetchWarlordGamesAllProducts(
    ...
    private static string? WarlordGamesGameSystem(ShopifyProduct product)
    {
    ...
    private static string? WarlordGamesFaction(ShopifyProduct product, string gameSystem)
    {
    ...
    private static async Task<IReadOnlyList<RawProduct>> FetchWyrdGamesAllProducts(
        bool verbose, CancellationToken ct)
    {
        using var source = new ShopifyProductSource(
    ...
    private static string? WyrdGamesGameSystem(ShopifyProduct product)
    {
    ...
    private static string? WyrdGamesFaction(ShopifyProduct product, string gameSystem)
    {
        if (product.Tags is null) return null;
    ...
    private static async Task<IReadOnlyList<RawProduct>> FetchManticGamesAllProducts(
        bool verbose, CancellationToken ct)
    {
    ...
    private static bool IsManticGameSystem(RawProduct product, string gameSystem)
    {
    ...
    private static async Task<IReadOnlyList<RawProduct>> FetchCorvusBelliProducts(
        string gameSystem, int sample, bool verbose, CancellationToken ct)
    {
    ...
    private static async Task<IReadOnlyList<RawProduct>> FetchAtomicMassGamesProducts(
        string gameSystem, int sample, bool verbose, CancellationToken ct)
    {
    ...
    private static async Task<IReadOnlyList<RawProduct>> FetchCmonProducts(
        string gameSystem, int sample, bool verbose, CancellationToken ct)
    {
    ...
    private static Task<IReadOnlyList<RawProduct>> FetchPrivateerPressProducts(
        string gameSystem, int sample, bool verbose, CancellationToken ct)
    {
    ...
    private static async Task<IReadOnlyList<RawProduct>> FetchSteamforgedGamesAllProducts(
        bool verbose, CancellationToken ct)
    {
    ...
    /// <summary>
    /// Holds the YAML deserializer shared by <see cref="LoadExistingFactionProductsAsync"/>, so it
    /// isn't rebuilt on every call (once per faction, per run).
    /// </summary>
    private static class ProductToolYaml
    {
