...
class DataPaths:
    ...
    def paint_eans(self) -> Path:
        """Every barcode the PAINT catalog publishes, generated by scripts/gen_paint_eans.py.

        The one place this resolver learns anything from the paint side, and it learns exactly one
        thing: whether a barcode is a paint's. Committed rather than read live so the layer
        boundary this module keeps against data/paints/ holds -- the same arrangement as
        data/catalog/set-contents/. Optional: absent, `category` falls back as it always did."""
...
def _load_paint_eans(path: Path) -> frozenset[str]:
    """The barcode KEYS of data/catalog/paint-eans.yaml; empty if the file is absent.

    Absent is legitimate, not an error: the package is tested outside the monorepo and every
    resolver fixture predates this file. The label it feeds is a strict addition, so with no index
    `category` falls back exactly as it did before -- which is also what makes the golden fixture
    and every unit test unchanged by this feature.

    Keys only. The values are audit labels for a human reading the file (which paint claimed this
    barcode); nothing in the resolver reads them, and membership is an exact string test -- the
    same comparison the publisher's cross-catalog join uses, "the canonical barcode string exactly
    as published. No normalization" (CrossCatalogLinks.cs).
    """
    if not path.exists():
        return frozenset()
    return frozenset(str(code) for code in ((read_yaml(path) or {}).get("eans") or {}))
...
def resolve_catalog(paths: DataPaths) -> dict[str, list[CanonicalProduct]]:
    ...
    paint_eans = _load_paint_eans(paths.paint_eans)
    ...
    for entity, members in joined.entities.items():
        ...
        record = resolve_attributes(
            ...
            member_codes=member_codes, paint_eans=paint_eans,
        )
