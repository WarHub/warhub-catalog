...
def _dump_product(record: CanonicalProduct) -> dict:
    ...
    for optional_list in ("additionalEans", "additionalCodes", "supersedes", "gameSystems", "settings"):
...
#
# `bundle-shared-barcode` -- a row filed under a DECLARED bundle's code (matches.yaml `bundles`)
#   carries the barcode of the product inside it. resolve/join.py leaves the barcode with the
#   component and drops it from the bundle's row, so two products stay two records and the barcode
#   scans to the thing it is printed on. It lasts as long as the maker's page quotes it.
#
...
#
# `shelf-manufacturer-corrected` -- a source whose `manufacturer` is the SHELF a row was filed
#   under (SourceDescriptor.manufacturerIsShelf) met, on the same product URL, a source that reads
#   that page's own vendor field. The maker won. Nothing is undecided and no edit removes a row: it
#   lasts as long as the reseller keeps stocking the brand. `matches.reassignManufacturer` is still
#   the tool for a single row a maintainer wants to correct by hand, and it outranks this.
#
...
def resolve_catalog(paths: DataPaths) -> dict[str, list[CanonicalProduct]]:
    ...
    # Declared bundles, alias-resolved and restricted to resolved entities the same way: the
    # bundle's record links to the product inside it (see Matches.bundles).
    bundle_of: dict[str, str] = {}
    for bundle, component in sorted(matches.bundles.items()):
        bundle = matches.aliases.get(bundle, bundle)
        component = matches.aliases.get(component, component)
        if bundle in joined.entities and component in joined.entities:
            bundle_of[bundle] = component
    for entity, members in joined.entities.items():
        ...
        ean = resolve_ean(
            entity, members, kinds, superseded, surviving_code=code, member_codes=member_codes,
            preferred=matches.preferEans.get(entity),
        )
        ...
        # THE MAKER'S OTHER CODE FOR THE SAME BOX, kept as a fact (Matches.codeAliases): every
        # alias a member of this record carried, minus the code that names it.
        record.additionalCodes = sorted(
            {joined.aliased_codes[m.key] for m in members if m.key in joined.aliased_codes} - {code}
        )
        record.bundleOf = bundle_of.get(entity)
    ...
    resolution_types = {
        ...
        "supersession-blocked-merge",
        "shelf-manufacturer-corrected",
        "bundle-shared-barcode",
    }
