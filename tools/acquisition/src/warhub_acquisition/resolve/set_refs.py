...
and only needs reading. Measured 2026-08-07 over all 22,529 committed products (11,503 with a
description): this rule selects 24 products, 90 enumerated lines, 62 distinct refs, and 0 products
outside `warlord-games`.
...
tomorrow" into a hand re-edit, and not scale to AK's own 256 boxed-set rows at all.

THE REF PATTERN IS AK-SPECIFIC ON PURPOSE, and that is a measurement rather than a preference.
Measured 2026-08-07 across all 11,503 committed descriptions: 0 products enumerate >=2 Army
Painter codes (WP/TL/ST/BR/GM/BF + 3-4 digits), 0 enumerate >=2 Vallejo `7x.xxx`, 0 enumerate >=2
Scale75 `SSE-`/`SFLOW-`. A brand-generic rule would earn nothing today and would put every other
brand's prose inside the blast radius. The pattern also excludes the corpus's one real false
positive: `AK-47`, in mantic-games/MGWD126 and mantic-games/MGWEB777 (Walking Dead equipment
cards) -- excluded twice over, by the 3-digit floor and by permitting no separator.

Deliberately NOT matched: `RC\\d{3}` (ak-real-color). 175 of that archive's 251 distinct codes are
duplicated across its 432 paints, so nearly every such ref would refuse anyway -- and measured,
not one AK set's prose names one.
"""
...
# members. Measured on committed data the anchor costs nothing -- it captures 90 of 90 tokens a
# free-floating `\bAK\d{3,5}\b` finds -- so it is pure future-proofing bought at zero present price.
#
...
# relation is for. Measured over the 999 live rows and all 11,503 committed descriptions, widening
# the ceiling changes exactly that one token.
_REF = r"AK\d{3,6}"

# THERE IS DELIBERATELY NO "Contains:" ANCHOR, and that is a reversal worth recording because it
# is the obvious design and it is wrong. Anchoring the scan to a contents header reads well on the
# 24 committed warlord sets, which all use a clean standalone "Contains:" line -- but a header
# anchor can only earn anything by SKIPPING member-shaped lines that precede it, and measured over
# every committed description AND the 999 live rows, the only lines it ever skipped were real
# members. AK8253 prints a late "INCLUDES:" sub-header (for the acetate sheet and paper posters)
# BELOW its six paints, and a first-header rule silently dropped all 9 of that set's refs. Removing
# the anchor changes exactly one set in the entire corpus, and changes it from 0 members to 9.
#
# What actually keeps prose out of the membership is the BULLET requirement in _MEMBER, not a
# header: live 2026-08-07, AK11621's copy says "...in miniature (previously AK3010)" mid-sentence,
# and that is excluded because it is not on a list item, with or without an anchor.

# STOP HERE, and this is the correction that matters most in this file. AK enumerates NOT-INCLUDED
# items in exactly the same bulleted, code-bearing shape as real contents. Measured 2026-08-07
# over the 256 sets, 6 carry such a marker and EVERY member-shaped line after it is a
# not-included item -- 20 lines in total: AK11701 "*These acrylic effects are not included:" (3
# bullets) and AK8252/8253/8254/8255 "OPTIONAL MATERIALS (not included)" (4, 5, 4, 4). A rule that
# simply harvested every list item in the block would publish those 20 as members of the box, which
# is a fabricated claim -- strictly worse than the missing member it is trying to avoid, because a
# refusal is visible and a false membership is not.
...
# Drab)</li>` for all 6 of its members. All 6 codes resolve to exactly one archive paint, and all
# 6 were dropped: the set then fell below the two-distinct-codes floor, `content_skus_from_
# description` returned None, and the refs appeared in NEITHER `members` NOR `unresolved` -- the
# exact vanishing the relation's own invariant forbids, and invisible because the set simply was
# not in the file.
#
...
# the moment a PAINT bullet carried one. Captured now: this is the one source measured in this
# repo that states a per-member quantity, and the relation file has reserved the key since it
# was written.
_MEMBER = re.compile(
...
def _first_language_block(description: str) -> str:
    ...
    THE SINGLE MOST LOAD-BEARING BOUND HERE. Live-verified 2026-08-07 on all 999 rows of AK's
    `paints-acrylics` category: `short_description` is a collapseomatic accordion carrying the
    FULL ENGLISH BLOCK AND THE FULL SPANISH BLOCK in one field, each with its own `<ul>` of the
    SAME codes under its own header ("Contains:" / "Contiene:"). All 256 boxed-set rows carry both
    markers and English is first in 256 of 256 -- which is also what the descriptor asks for
    (`scope.extraParams.lang: en`). Without this cut every AK set would report its members exactly
    twice, and gen_set_contents.py's within-set repeat rule would then refuse half of them.
...
def enumerated_members(description: str | None) -> list[tuple[str, str, int | None]]:
    ...
    a single contiguous run: 11 of the 256 AK sets split their contents over several lists under
    sub-headings ("3Gen Acrylics:", "Enamel Effects:", "Brushes:"), and a run-based rule truncated
    AK11757 and AK11763 at 3 members of 18. Interleaved prose is skipped rather than treated as a
    terminator, which also means one malformed bullet no longer silently ends the list --
    measured, AK11781's "AK111424" typo did exactly that.
...
def content_skus_from_description(description: str | None) -> list[str] | None:
    ...
    for every singles page that mentions a companion pot.

    HONESTLY, THE BAR CATCHES NOTHING TODAY and is kept anyway. Measured 2026-08-07 it changes no
    outcome on either population: all 24 committed products that yield refs carry >=2 distinct
    codes, and across AK's 256 live boxed-set rows exactly 0 print a single code (210 print >=2,
    46 print none). It stays because the failure it guards against is the one that would be
    invisible -- a one-item "set" published off a cross-reference reads exactly like a real
    membership -- and because it costs nothing to keep.
    """
